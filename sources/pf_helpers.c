...
static char *pf_prec(t_format *f, char **str)
{
	...
	add = (f->alt && **str != 48) || (f->opt >= 2 && f->base != 10) ?
		f->base == 16 ? f->opt & 1 ? "0x" : "0X" : "0" : "";
	lother = MAX((lstr = (int)ft_strlen(*str)), f->prec) + (int)ft_strlen(add);
	if (f->prec == 0 && **str == 48)
		tmp = ft_strdup(f->width > 0 ? " " : f->base == 8 && f->alt ?
			"0" : add);
	else if ((tmp = ft_strnew(lother)))
	{
		*tmp = (i = (f->pad == 48|| f->prec > 0) && SIGN(**str)) ? **str : 0;
		SCAT(MSET(SCAT(tmp + i, add), 48, MAX(f->prec - lstr, 0)), *str + i);
	}
	else
		return (*str);
	free (*str);
...
char *pf_fill(t_format *f, char **str)
{
	...
	len = (lstr = (int)ft_strlen(*str)) < MAX(f->width, f->prec) ?
